...
* `--report-type`: If specified, a report will be created on disk for the current test run. `junit` is currently the only support type. You need to specify the `--report-output-url` path for the report.
* `--report-output-url`: The path to the report file (existing or not).
